...
if not fetchSuccess or type(SecureData) ~= "table" then
    game:GetService("Players").LocalPlayer:Kick("[7hub] Failed to load secure module. Please check your connection.")
    return
end

-- Lấy dữ liệu từ blackbox
local service     = SecureData.s
local secret      = SecureData.k
local MAIN_SCRIPT = SecureData.m
